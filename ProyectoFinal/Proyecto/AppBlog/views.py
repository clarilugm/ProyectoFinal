...
def editarTitulo(request, id):
    if request.method == 'POST':
        titulo = Blog.objects.get(id=id)
        form = BlogForm(request.POST, request.FILES)
        if form.is_valid():
            titulo.titulo = form.cleaned_data['titulo']
            titulo.subtitulo = form.cleaned_data['subtitulo']
            titulo.fecha = form.cleaned_data['fecha']
            titulo.contenido = form.cleaned_data['contenido']
            titulo.imagen = form.cleaned_data['imagen']
            titulo.save()
        ...
        titulos = Blog.objects.get(id=id)
        form = BlogForm(initial={'titulo':titulo.titulo, 'subtitulo': titulo.subtitulo, 'fecha':titulo.fecha, 'contenido':titulo.contenido, 'imagen':titulos.imagen})
...
def detallesTitulo(request, id):
    if request.method == 'GET':
        blog = Blog.objects.filter(id=2)
        if blog:
            titulo = Blog.objects.get(id=2)
