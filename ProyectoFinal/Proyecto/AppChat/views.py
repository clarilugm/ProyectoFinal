...
def inicioMensajes(request, id=None):
    if request.method == 'GET':
        ...
        if id != None: 
            ...
            return render(request, 'AppChat/inicioMensaje.html', {'usuarios':usuarios, 'avatar':obtenerAvatar(request) , 'formMensaje':formMensaje, 'receptor': receptor, 'chat':chat})
        else:
            formMensaje = mensajeForm()
            usuarios = User.objects.all()
        ...
        if formMensaje.is_valid() and id != None:
            ...
            formMensaje = mensajeForm()
            usuarios = User.objects.all()
            return render(request, 'AppChat/inicioMensaje.html', {'usuarios':usuarios, 'avatar':obtenerAvatar(request) , 'formMensaje':formMensaje, 'mensaje':'Mensaje enviado correctamente!'})
        else:
            formMensaje = mensajeForm()
            usuarios = User.objects.all()
            return render(request, 'AppChat/inicioMensaje.html', {'usuarios':usuarios, 'avatar':obtenerAvatar(request) , 'formMensaje':formMensaje, 'mensaje':'Error al enviar el mensaje, su formulario no es valido'})
    return render(request, 'AppChat/inicioMensaje.html', {'usuarios':usuarios, 'avatar':obtenerAvatar(request) , 'formMensaje':formMensaje})
